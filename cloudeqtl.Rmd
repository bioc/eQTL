...
for each of the loci.  Select the search radius &rho \\(\rho\\) (for example, 100kb) and
...
<h2 id="parallel-sec">Programming the parallelized search: various approaches</h2>

<h3 id="hilev-sec"> High-level, socket-based cluster: ciseqByCluster </h3>
...
<h3 id="ovplot-sec">Overview QQ-plot</h3>
...
<h3 id="sens-sec">Sensitivity analysis of tuning the search</h3>
...
library(doParallel)
library(parallel)
dcdown = max(c(detectCores()-1,1))
registerDoParallel(cores=min(c(dcdown,5)))
...
<h3 id="gviz-sec">Visualizing results for a gene</h3>
